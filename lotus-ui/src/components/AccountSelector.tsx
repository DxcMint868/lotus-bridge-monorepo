...
const AccountSelector = ({ onSelectAccount, selectedAddress, disabled }: AccountSelectorProps) => {
  ...
  const selectedAccount = accounts.find(acc => acc.address.toLowerCase() === selectedAddress?.toLowerCase());
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DropdownMenuContent align="start" className="w-72 glass-card">
          ...
          {accounts.map((account, index) => (
            <DropdownMenuItem
              key={account.address}
              className="flex items-center justify-between p-3 cursor-pointer hover:bg-white/20 focus:bg-white/20"
