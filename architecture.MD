...
```mermaid
graph TD
    subgraph "A. Application Layer (User-Facing)"
        direction LR
        UI[User Interface]
        API[️Bridge API]
    end

    subgraph "B. Protocol Layer (On-Chain Logic)"
        direction LR
    ...
    subgraph "C. Messaging & Infrastructure Layer (Cross-Chain Communication)"
        direction LR
